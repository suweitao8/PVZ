...
**自动提交规则：**
完成每个需求后，必须自动执行 git 提交，步骤如下：

1. `git add` 暂存本次需求相关的文件（不要 `git add -A`，排除无关文件）
2. `git commit` 使用 Conventional Commits 格式提交（`feat:` / `fix:` / `refactor:` / `docs:` 等）
3. 提交信息用中文描述变更内容

不要询问是否需要提交，需求完成后直接提交。
