...
public static class CardPileCmd
{
	...
	{
		await RemoveFromDeck(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardModel>(card), showPreview);
	}
	...
	{
		await RemoveFromCombat(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardModel>(card), skipVisuals);
	}
	...
	{
		return (await AddGeneratedCardsToCombat(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardModel>(card), newPileType, addedByPlayer, position))[0];
	}
	...
	{
		return (await Add(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardModel>(card), newPile, position, source, skipVisuals))[0];
	}
	...
	{
		await AddCursesToDeck(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardModel>(ModelDb.Card<T>()), owner);
	}
