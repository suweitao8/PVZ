...
public class RunState : IRunState, ICardScope, IPlayerCollection
{
	...
	public static RunState FromSerializable(SerializableRun save)
	{
		...
		runState._mapPointHistory.AddRange(new global::MegaCrit.Sts2.Core.Collections.ReadOnlyArray<List<MapPointHistoryEntry>>(save.MapPointHistory.ToArray()));
	...
	public static RunState CreateForTest(IReadOnlyList<Player>? players = null, IReadOnlyList<ActModel>? acts = null, IReadOnlyList<ModifierModel>? modifiers = null, int ascensionLevel = 0, string? seed = null)
	{
		...
		RunRngSet runRngSet = new RunRngSet(seed);
		RunState runState = CreateShared(players ?? new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<Player>(Player.CreateForNewRun<Deprived>(MegaCrit.Sts2.Core.Unlocks.UnlockState.all, 1uL)), (acts ?? ActModel.GetDefaultList()).Select((ActModel a) => a.ToMutable()).ToList(), modifiers ?? Array.Empty<ModifierModel>(), 0, runRngSet, new RunOddsSet(runRngSet.UnknownMapPoint), new RelicGrabBag(refreshAllowed: true), ascensionLevel);
	...
	public void AddModifierDebug(ModifierModel modifier)
	{
		...
		array[num] = modifier;
		Modifiers = new global::MegaCrit.Sts2.Core.Collections.ReadOnlyArray<ModifierModel>(array);
	}
