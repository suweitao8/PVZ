...
public class CrystalSphereMinigame
{
	...
	{
		return GetHorizontalCells(x, y).Concat(GetVerticalCells(x, y)).Concat(GetDiagonalCells(x, y)).Concat(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<Vector2I>(new Vector2I(x, y)))
			.ToList();
	}
