...
public sealed class ScrollBoxes : RelicModel
{
	...
	public static List<IReadOnlyList<CardModel>> GenerateRandomBundles(Player player)
	{
		...
		CardCreationOptions options = CardCreationOptions.ForNonCombatWithUniformOdds(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardPoolModel>(player.Character.CardPool), (CardModel c) => c.Rarity == CardRarity.Common).WithFlags(CardCreationFlags.NoRarityModification);
		options = Hook.ModifyCardRewardCreationOptions(player.RunState, player, options);
		CardCreationOptions options2 = CardCreationOptions.ForNonCombatWithUniformOdds(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardPoolModel>(player.Character.CardPool), (CardModel c) => c.Rarity == CardRarity.Uncommon).WithFlags(CardCreationFlags.NoRarityModification);
		...
		for (int num = 0; num < 2; num++)
		{
			if (flag && rewards.NextInt(100) < 1)
			{
				...
				list.Add(new global::MegaCrit.Sts2.Core.Collections.ReadOnlyArray<CardModel>(new CardModel[3] { cardModel, cardModel, cardModel }));
				continue;
			}
