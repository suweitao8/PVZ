	...
	public ModelId? AncientCard
	{
		...
		set
		{
			...
			if (_ancientCard != null)
			{
				...
				_extraHoverTips = cardModel.HoverTips.Concat(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.FromCard(cardModel, upgrade: true)));
				((StringVar)base.DynamicVars["AncientCard"]).StringValue = cardModel.Title;
			}
		}
	}

	protected override IEnumerable<DynamicVar> CanonicalVars => new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<DynamicVar>(new StringVar("AncientCard"));
