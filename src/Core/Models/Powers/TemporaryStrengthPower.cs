	...
	protected override IEnumerable<IHoverTip> ExtraHoverTips
	{
		get
		{
			...
			if (!(originModel is CardModel card))
			{
				...
				{
					collection = new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.FromPotion(model));
				}
			}
			else
			{
				collection = new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.FromCard(card));
			}
			list2.AddRange(collection);
			list.Add(HoverTipFactory.FromPower<StrengthPower>());
			return new Core.Collections.ReadOnlyList<IHoverTip>(list);
		}
	}
