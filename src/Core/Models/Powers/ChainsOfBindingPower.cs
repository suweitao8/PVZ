...
public sealed class ChainsOfBindingPower : PowerModel
{
	...
	public override async Task AfterCardDrawn(PlayerChoiceContext choiceContext, CardModel card, bool fromHandDraw)
	{
		if (card.Owner == base.Owner.Player && base.CombatState.CurrentSide == base.Owner.Side && ModelDb.Affliction<Bound>().CanAfflict(card))
		{
			...
			if (num < base.Amount)
			{
				await CardCmd.AfflictAndPreview<Bound>(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardModel>(card), base.Amount, CardPreviewStyle.None);
			}
		}
	}
