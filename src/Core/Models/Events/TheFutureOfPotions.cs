...
public sealed class TheFutureOfPotions : EventModel
{
	...
	private async Task Trade(PotionModel potion)
	{
		...
		CardCreationOptions options = CardCreationOptions.ForNonCombatWithUniformOdds(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardPoolModel>(base.Owner.Character.CardPool), (CardModel c) => c.Rarity == targetRarity && c.Type == PotionToCardType[potion]).WithFlags(CardCreationFlags.NoRarityModification);
