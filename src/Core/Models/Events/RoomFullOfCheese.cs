...
{
	protected override IEnumerable<DynamicVar> CanonicalVars => new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<DynamicVar>(new DamageVar(14m, ValueProp.Unblockable | ValueProp.Unpowered));
	...
	private async Task Gorge()
	{
		...
		CardCreationOptions options = CardCreationOptions.ForNonCombatWithUniformOdds(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardPoolModel>(owner.Character.CardPool), (CardModel c) => c.Rarity == CardRarity.Common).WithFlags(CardCreationFlags.NoRarityModification);
