...
public sealed class BrainLeech : EventModel
{
	...
	protected override IEnumerable<DynamicVar> CanonicalVars => new global::MegaCrit.Sts2.Core.Collections.ReadOnlyArray<DynamicVar>(new DynamicVar[4]
	{
	...
	private async Task ShareKnowledge()
	{
		...
		List<CardCreationResult> cards = CardFactory.CreateForReward(owner, base.DynamicVars["FromCardChoiceCount"].IntValue, CardCreationOptions.ForNonCombatWithDefaultOdds(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardPoolModel>(owner.Character.CardPool))).ToList();
