...
public sealed class EndlessConveyor : EventModel
{
	...
	protected override IEnumerable<DynamicVar> CanonicalVars => new global::MegaCrit.Sts2.Core.Collections.ReadOnlyArray<DynamicVar>(new DynamicVar[6]
	{
		new GoldVar(35),
	...
	{
		return new global::MegaCrit.Sts2.Core.Collections.ReadOnlyArray<EventOption>(new EventOption[2]
		{
	...
	private async Task GrabSomethingOffTheBelt()
	{
		...
		((StringVar)base.DynamicVars["CurrentDishTitle"]).StringValue = CurrentDish.title.GetFormattedText();
		SetEventState(L10NLookup("ENDLESS_CONVEYOR.pages.GRAB_SOMETHING_OFF_THE_BELT.description"), new global::MegaCrit.Sts2.Core.Collections.ReadOnlyArray<EventOption>(new EventOption[2]
		{
			GenerateGrabSomethingOffTheBeltOption(),
	...
	{
		CardCreationOptions options = CardCreationOptions.ForNonCombatWithDefaultOdds(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<CardPoolModel>(ModelDb.CardPool<ColorlessCardPool>()));
	...
	private void RollDish()
	{
		...
		num2++;
		span[num2] = new Dish("JELLY_LIVER", JellyLiver, new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.Static(StaticHoverTip.Transform)), 3f);
		num2++;
