...
{
	protected override IEnumerable<DynamicVar> CanonicalVars => new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<DynamicVar>(new DamageVar(8m, ValueProp.Move));
	...
	protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
		...
		{
			List<Creature> list2 = (from e in base.CombatState.GetTeammatesOf(damageResult.Receiver).Except(new global::MegaCrit.Sts2.Core.Collections.ReadOnlySingleElementList<Creature>(cardPlay.Target))
				where e.IsHittable
				select e).ToList();
			if (list2.Count != 0)
			{
